...
class NaverStoreFAQSettings:
    ...
    def __init__(self, db_data_path, db_collection_path, **kwargs):
        ...
        self.COLLECTION_PATH = (
            f"{db_collection_path}/{self.NAVER_STORE_FAQ_COLLECTION_NAME}.pkl"
        )
...
class ChromaDBSettings:
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.NAVER_STORE_FAQ = NaverStoreFAQSettings(
            self.DB_DATA_PATH, self.DB_COLLECTION_PATH
        )
        self.collections = self._initialize_collections()

    def _initialize_collections(self) -> Dict[str, Any]:
        collections = {}
        for attr_name, _attr_type in self.__annotations__.items():
